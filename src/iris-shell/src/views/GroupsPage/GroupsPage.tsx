...
export function GroupsPage() {
  ...
  const rowMenuItems = (group: Group): MenuEntry[] => [
    ...
    { kind: 'item', label: 'Move', icon: 'Folder', onSelect: () => { setSelected(new Set([group.id])); setMoveGroupsOpen(true); } },
    { kind: 'divider' },
  ...
  return (
    <AppShell breadcrumb={[{ label: 'Directory Management' }, { label: 'Groups' }]}>
      ...
      <ActionBar open={selected.size > 0} selectedCount={selected.size} totalCount={rows.length} onDismiss={() => setSelected(new Set())} groups={[[{ icon: 'Copy', label: 'Copy', onClick: () => copyGroups(selectedGroups) }, { icon: 'Folder', label: 'Move', onClick: () => setMoveGroupsOpen(true) }, { icon: 'Trash', label: 'Delete', tone: 'danger', onClick: () => setDeleteGroup({ id: `selection-${Date.now()}`, name: `${selected.size} groups`, status: 'Active', description: '', members: 0, location: '', scope: 'Global' }) }]]} />
      <NewGroupModal
        open={newGroupOpen}
        directories={[
          ['entra-1', 'Entra 1'],
      ...
      <MoveGroupsModal
        ...
        onMove={(directory) => {
          ...
          setSelected(new Set());
          showToast(`${selectedGroups.length} group${selectedGroups.length === 1 ? '' : 's'} moved.`);
        }}
      />
    </AppShell>
  );
